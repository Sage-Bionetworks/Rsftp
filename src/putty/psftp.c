...
{
    Rprintf("psftp: not connected to a host; use \"open host.name\"\n");
}
...
int sftp_get_file(char *fname, char *outfname, int recurse, int restart)
{
	...
	if (recurse) {
		...
		if (result &&
			...
			if (file_type(outfname) != FILE_TYPE_DIRECTORY &&
				...
				Rprintf("%s: Cannot create directory\n", outfname);
				return 0;
			}
			...
			if (!dirhandle) {
			 Rprintf("%s: unable to open directory: %s\n",
						fname, fxp_error());
				return 0;
			}
			nnames = namesize = 0;
				...
				for (i = 0; i < names->nnames; i++)
					if (strcmp(names->names[i].filename, ".") &&
						...
						if (!vet_filename(names->names[i].filename)) {
						 Rprintf("ignoring potentially dangerous server-"
									"supplied filename '%s'\n",
	...
	if (restart) {
		...
		if (seek_file(file, uint64_make(0,0) , FROM_END) == -1) {
			close_wfile(file);
			Rprintf("reget: cannot restart %s - file too large\n",
					outfname);
			req = fxp_close_send(fh);
			pktin = sftp_wait_for_reply(req);
			fxp_close_recv(pktin, req);
		...
		Rprintf("reget: restarting at file position %s\n", decbuf);
	} else {
	...
	while (!xfer_done(xfer)) {
		...
		while (xfer_download_data(xfer, &vbuf, &len)) {
			...
			while (wpos < len) {
				...
				if (wlen <= 0) {
				 Rprintf("error while writing local file\n");
					ret = 0;
					xfer_set_error(xfer);
					break;
				}
...
int sftp_put_file(char *fname, char *outfname, int recurse, int restart)
{
	...
	if (!dh) {
	    Rprintf("%s: unable to open directory\n", fname);
	    return 0;
	}
	...
	if (!ret) {
	    close_rfile(file);
	    Rprintf("read size of %s: %s\n", outfname, fxp_error());
	    return 0;
	}
	if (!(attrs.flags & SSH_FILEXFER_ATTR_SIZE)) {
	    close_rfile(file);
	    Rprintf("read size of %s: size was not given\n", outfname);
	    return 0;
	}
	offset = attrs.size;
	uint64_decimal(offset, decbuf);
	Rprintf("reput: restarting at file position %s\n", decbuf);
	...
	while (xfer_upload_ready(xfer) && !err && !eof) {
	    ...
	    if (len == -1) {
	    	Rprintf("error while reading local file\n");
	    	err = 1;
	    } else if (len == 0) {
		eof = 1;
	    } else {
...
SftpWildcardMatcher *sftp_begin_wildcard_matching(char *name)
{
    ...
    if (!check) {
    	Rprintf("Multiple-level wildcards are not supported\n");
	sfree(unwcdir);
	return NULL;
    }
...
{
    Rprintf("psftp: unknown command \"%s\"\n", cmd->words[0]);
    return 0;			       /* failure */
}
...
int sftp_cmd_cd(struct sftp_command *cmd)
{
    ...
    sfree(pwd);
    pwd = dir;
    Rprintf("Remote directory is now %s\n", pwd);
...
int sftp_general_get(struct sftp_command *cmd, int restart, int multiple)
{
	...
	if (!strcmp(cmd->words[i], "--")) {
	    ...
	    Rprintf("%s: unrecognised option '%s'\n", cmd->words[0], cmd->words[i]);
	    return 0;
	}
	i++;
    }
...
int sftp_cmd_mkdir(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 2) {
    	Rprintf("mkdir: expects a directory\n");
    	return 0;
    }
...
int sftp_cmd_rmdir(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 2) {
    Rprintf("rmdir: expects a directory\n");
	return 0;
    }
...
int sftp_cmd_mv(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 3) {
 Rprintf("mv: expects two filenames\n");
	return 0;
    }
 ...
 Rprintf("mv: multiple or wildcard arguments require the destination"
	       " to be a directory\n");
	sfree(ctx->dstfname);
	return 0;
    }
...
int sftp_cmd_chmod(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 3) {
 Rprintf("chmod: expects a mode specifier and a filename\n");
	return 0;
    }
	...
	if (mode[strspn(mode, "01234567")]) {
	    Rprintf("chmod: numeric file modes should"
	...
	while (*mode) {
		...
		switch (*mode) {
		  ...
		  default:
		    Rprintf("chmod: file mode '%.*s' contains unrecognised"
	    ...
	    if (!*mode || *mode == ',') {
	 Rprintf("chmod: file mode '%.*s' is incomplete\n",
	 ...
	 Rprintf("chmod: file mode '%.*s' is incomplete\n",
		...
		switch (*mode) {
		  ...
		  case 's':
			...
			(subset & 06777) != 02070) {
		 Rprintf("chmod: file mode '%.*s': set[ug]id bit should"
	 ...
	 Rprintf("chmod: file mode '%.*s' contains no user/group/other"
...
static int sftp_cmd_open(struct sftp_command *cmd)
{
    ...
    if (back != NULL) {
 Rprintf("psftp: already connected\n");
	return 0;
    }

    if (cmd->nwords < 2) {
 Rprintf("open: expects a host name\n");
	return 0;
    }
	...
	if (portnumber == 0) {
	    Rprintf("open: invalid port number\n");
	    return 0;
	}
    } else
	portnumber = 0;
...
static int sftp_cmd_lcd(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 2) {
 Rprintf("lcd: expects a local directory name\n");
	return 0;
    }

    errmsg = psftp_lcd(cmd->words[1]);
    if (errmsg) {
 Rprintf("lcd: unable to change directory: %s\n", errmsg);
	sfree(errmsg);
	return 0;
    }

    currdir = psftp_getcwd();
    Rprintf("New local directory is %s\n", currdir);
    sfree(currdir);
...
static int sftp_cmd_lpwd(struct sftp_command *cmd)
{
    ...
    currdir = psftp_getcwd();
    Rprintf("Current local directory is %s\n", currdir);
    sfree(currdir);
...
static int sftp_cmd_help(struct sftp_command *cmd)
{
	...
	for (i = 0; i < sizeof(sftp_lookup) / sizeof(*sftp_lookup); i++) {
	    ...
	    Rprintf("%-*s", maxlen+2, lookup->name);
	...
	for (i = 1; i < cmd->nwords; i++) {
	    ...
	    if (!lookup) {
	 Rprintf("help: %s: command not found\n", cmd->words[i]);
	    } else {
	 Rprintf("%s", lookup->name);
		if (lookup->longhelp == NULL)
		    lookup = lookup_command(lookup->shorthelp);
	 Rprintf("%s", lookup->longhelp);
	    }
	}
    }
    return 1;
...
static int do_sftp_init(void)
{
     ...
     */
    if (!fxp_init()) {
    	REprintf("Fatal: unable to initialise SFTP: %s\n", fxp_error());
	return 1;		       /* failure */
    }
    ...
    if (!homedir) {
    	REprintf("Warning: failed to resolve home directory: %s\n",
		fxp_error());
    	homedir = dupstr(".");
    } else {
    	/*Rprintf("Remote working directory is %s\n", homedir);*/
    }
    pwd = dupstr(homedir);
    return 0;
}
...
void do_sftp(int mode, int modeflags, char *batchfile)
{
	...
	if (mode == 0) {
		...
		if (!fp) {
		 Rprintf("Fatal: unable to open %s\n", batchfile);
			return;
		}
		while (1) {
			struct sftp_command *cmd;
...
{
    Rprintf("PuTTY Secure File Transfer (SFTP) client\n");
    Rprintf("%s\n", ver);
    Rprintf("Usage: psftp [options] [user@]host\n");
    Rprintf("Options:\n");
    Rprintf("  -V        print version information and exit\n");
    Rprintf("  -pgpfp    print PGP key fingerprints and exit\n");
    Rprintf("  -b file   use specified batchfile\n");
    Rprintf("  -bc       output batchfile commands\n");
    Rprintf("  -be       don't stop batchfile processing if errors\n");
    Rprintf("  -v        show verbose messages\n");
    Rprintf("  -load sessname  Load settings from saved session\n");
    Rprintf("  -l user   connect with specified username\n");
    Rprintf("  -P port   connect to specified port\n");
    Rprintf("  -pw passw login with specified password\n");
    Rprintf("  -1 -2     force use of particular SSH protocol version\n");
    Rprintf("  -4 -6     force use of IPv4 or IPv6\n");
    Rprintf("  -C        enable compression\n");
    Rprintf("  -i key    private key file for authentication\n");
    Rprintf("  -noagent  disable use of Pageant\n");
    Rprintf("  -agent    enable use of Pageant\n");
    Rprintf("  -batch    disable all interactive prompts\n");
    cleanup_exit(1);
}
...
int psftp_connect(char *userhost, char *user, int portnumber)
{
	...
	*host++ = '\0';
	if (user) {
	    Rprintf("psftp: multiple usernames specified; using \"%s\"\n",
		   user);
	} else
	    user = userhost;
    }
...
int psftp_main(int argc, char *argv[])
{
 ...
 Rprintf("psftp: no hostname specified; use \"open host.name\""
	       " to connect\n");
    }
